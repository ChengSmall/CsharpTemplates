...
namespace Cheng.Texts
{
    ...
    /// 可变字符串范围读取器
    /// </summary>
    public class CMStringBuilderReader : SafeReleaseTextReader
    {
        ...
        public CMStringBuilderReader(strBuf value, int index, int count)
        {
            if (value is null)
            {
                p_strBuf = value;
        ...
        /// </summary>
        /// <param name="value">要读取的字符串</param>
        public CMStringBuilderReader(strBuf value)
        {
            p_strBuf = value;
            p_index = 0;
