...
using FPT = Cheng.DataStructure.FixedPoints.FixedPoint32I24P8;
...
namespace Cheng.DataStructure.FixedPoints
{
    ...
    /// <summary>
    /// 32位定点数 - 整数位24bit 小数位8bit
    /// </summary>
    /// <remarks>
    /// <para>
    /// 32位定点数，采用4字节整数最为基本数据，前8bit是小数位，剩余24bit是整数位，共同组合成为定点小数<br/>
    /// 位结构如下所示，使用G作为整数bit，X作为小数bit，N作为符号位:<br/>
    /// <code>NGGGGGGG GGGGGGGG GGGGGGGG.XXXXXXXX</code>
    /// </para>
    /// <para>定点数的运算结果具有严格的跨平台一致性</para>
    /// </remarks>
    public readonly unsafe struct FixedPoint32I24P8 : IEquatable<FPT>, IComparable<FPT>
    {
        ...
        public FixedPoint32I24P8(FPRAW raw)
        {
            this.raw = raw;
        }
...
#if DEBUG
        /// <summary>
        /// ln(2) ≈ 0.6931，Q8格式下约为 0.6931 * 256 ≈ 177
        /// </summary>
#endif
        private const int Ln2Q8 = 177;
        ...
        private static FPT f_naturalLog(FPT x)
        {
            ...
            return new FPT((FPRAW)(lnMantissa + exponent * Ln2Q8));
        }
        ...
        private static FPT f_exp(FPT x)
        {
            ...
            int k = (int)xAbsolute / Ln2Q8;
            int remainder = (int)xAbsolute - k * Ln2Q8;
        ...
        public static FPT Pow(FPT x, FPT y)
        {
            ...
            return f_exp(Mult(y, f_naturalLog(x)));
        }
        ...
        public static FPT Sqrt(FPT num)
        {
            ...
            // 推导：设结果为Q8格式，则 result_raw = sqrt(raw * 2^8)
            return new FPT(IntegerSqrt(num.raw << FractionalBits));
        }
        ...
        private unsafe static FPT f_createFromStrBuf(char* str, int length)
        {
            ...
            if (pos < length && str[pos] == '.')
            {
                pos++;
                // 最多解析9位小数
                while ((pos < length) && char.IsDigit(str[pos]) && (fractionalDigits < 9))
                {
        ...
        /// <summary>
        /// 将值等价转换到<see cref="FixedPoint32I22P10"/>
        /// </summary>
        /// <returns></returns>
        ...
        /// </summary>
#endif
        private const int FractionalDecMul = 390625;
        ...
        private string f_tostr()
        {
            ...
            {
                // 8位十进制小数的整数值
                int fracDec = fractionalPart * FractionalDecMul;
                // 前导零8位固定长度
                string fracStr = fracDec.ToString("D8");
        ...
        private string f_toStringFixed()
        {
            ...
            return $"{(isNegative ? "-" : "")}{integerPart}.{fracDec:D8}";
        }
